	...
	const getFilteredPlayers = () => {
		...
		if (analysisFilter === "playing")
			filtered = filtered.filter((s) =>
				players.find((p) => p.number === s.number && p.isPlaying)
			);
		else if (analysisFilter !== "all")
			filtered = filtered.filter((s) =>
				players.find(
					(p) => p.number === s.number && p.position === analysisFilter
				)
			);
		return filtered;
	};
	...
	const createGradientOverlay = () => {
		...
		for (let y = 0; y < 100; y += gridSize) {
			for (let x = 0; x < 100; x += gridSize) {
				const nearby = ballDrops.filter(
					(d) =>
						Math.hypot(d.x - (x + gridSize / 2), d.y - (y + gridSize / 2)) < 15
				);
				if (nearby.length)
					gradientData.push({
						x,
						y,
						intensity: Math.min(nearby.length * 0.3, 1),
					});
			}
		}
